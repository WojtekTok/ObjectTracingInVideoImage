...
# === ARGUMENTY (możesz zamienić na config lub argparse jeżeli chcesz) ===
MODEL_PATH = '../experiments/siammask_sharp/SiamMask_DAVIS.pth'   # <-- podaj swój model
CONFIG_PATH = '../experiments/siammask_sharp/config_davis.json'
USE_CPU = False

app = Flask(__name__)

# === GLOBALNY STAN TRACKERA ===
tracker_state = None
siammask = None
cfg = None
device = None
...
def api_init():
    ...
    # Odbierz plik obrazu i bbox
    file = request.files['image']
    bbox = request.form['bbox']  # spodziewamy się stringa: "x,y,w,h"
    x, y, w, h = map(float, bbox.split(','))

    # Zamiana pliku obrazu na np.array
